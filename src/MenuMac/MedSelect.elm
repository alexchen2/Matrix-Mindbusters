module MenuMac.MedSelect exposing (..)
...
import Misc.LvlImgs exposing ( medLevel1, medLevel2, medLevel3, medLevel4 )

{- ** Constants ** -}
-- Change below two constants (and some alg data types below) to switch menu for different difficulties
numLevels = 4
levelPrefix = "M"             -- "E" for Easy, "M" for Mediumium, "H" for Hard
...
levelImgList = [medLevel1, medLevel2, medLevel3, medLevel4] 
...
-- Originally intended to make module more dynamic and choose from Easy, Mediumium, or Medium, but scrapped that idea
type Level = Medium Int
           | LevelNone
...
init = { time = 0 
       ...
       , hoverExit = False 
       , click = ClickNone
       , levelDisplay = Medium 1
       , prevLevelDisplay = Medium 1   -- LevelNone
       , levelChoose = LevelNone       
       , winState = Opening 
       , isExit = False }

main = gameApp Tick { model = init, view = view, update = update, title = "MM | Select a Medium Level" }
