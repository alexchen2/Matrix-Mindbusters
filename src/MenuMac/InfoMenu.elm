module MenuMac.InfoMenu exposing (..)
...
-- Local imports
import Misc.MatrixStrings exposing (strSkew, strStretch, strScale, strTrans, strRot, 
                                    demoGifSkew, demoGifStretch, demoGifScale, demoGifTrans, demoGifRot)
...
space = boxWidth / 5
tab1X = ( (-(boxWidth / 2) + 1, 0), (-(boxWidth / 2) + space + (3/5), 0) )
tab2X = ( (-(boxWidth / 2) + space + (3/5), 0), (-(boxWidth / 2) + (space * 2) + (1/5), 0) )
...
textSkew = 
  strParser strSkew textSize1 textSpacing1
    |> move (-(boxWidth / 2) + 5, 13)
    |> scale 0.95
    
textStretch = 
  strParser strStretch textSize1 textSpacing1
    |> move (-(boxWidth / 2) + 5, 14)
    |> scale (0.95)
    
textScale =
  strParser strScale textSize1 textSpacing1
    |> move (-(boxWidth / 2) + 5, 12)
    |> scale (0.95)
    
textRot =
  strParser strRot textSizeRot textSpacingRot
    |> move (-(boxWidth / 2) + 5, 7.5)
    |> scale (0.95)
    
textTrans =
  strParser strTrans textSize1 textSpacing1
    |> move (-(boxWidth / 2) + 5, 14)
    |> scale (0.95)
    
gifSkew = 
  [ demoGifSkew
  , roundedRect (499 * 0.15) (298 * 0.15) 0.5
      |> outlined (solid 0.5) boxClr
      |> move (37.425, -22.35)
  ]
    |> group
    |> move (3, 11)
...
{- Takes a block of string with every new line separated by \n and converts it
 ...
 -}
strParser : String -> Float -> Float -> Shape userMsg
strParser string textSize textSpacing = 
  let
...
body model = 
  case model.tabState of
    Tab1 -> [ textSkew, gifSkew ] 
              |> group
    Tab2 -> [ textStretch, gifStretch ]
              |> group
    Tab3 -> [ textScale, gifScale]
              |> group
    Tab4 -> [ textRot, gifRot ]
              |> group
    Tab5 -> [ textTrans, gifTrans ]
              |> group
    _ -> text "" |> ghost
...
tabOpenLbl coords =
  if coords == tab1X then 
    ...
    lbl2
  else if coords == tab3X then 
    lbl3
  else if coords == tab4X then 
    lbl4
  else if coords == tab5X then 
    lbl5
  else 
    ""
...
tabLbls = 
  let
    ...
    lblList = [ (lbl1, ((first (first tab1X)) + (first (second tab1X))) / 2)
              ...
              , (lbl3, ((first (first tab3X)) + (first (second tab3X))) / 2)
              , (lbl4, ((first (first tab4X)) + (first (second tab4X))) / 2)
              , (lbl5, ((first (first tab5X)) + (first (second tab5X))) / 2) ]
  in
...
tabVertLines openCoords = 
  let
    ...
    xCoords = [ -(boxWidth / 2) + space + (3/5)
              , -(boxWidth / 2) + (space * 2) + (1/5)
              , (boxWidth / 2) - (space * 2) - (1/5)
              , (boxWidth / 2) - space - (3/5) ]
  in
...
tabCollision model =
   ( [ polygon [ (-(boxWidth / 2) + 1, 10)
             , (-(boxWidth / 2) + 1, 0)
             , (-(boxWidth / 2) + space + (3/5), 0)
             , (-(boxWidth / 2) + space + (3/5), 10) ]
   ...
   , polygon [ (-(boxWidth / 2) + space + (3/5), 10)
             , (-(boxWidth / 2) + space + (3/5), 0)
   ...
   , polygon [ (-(boxWidth / 2) + (space * 2) + (1/5), 10)
             , (-(boxWidth / 2) + (space * 2) + (1/5), 0)
             , ((boxWidth / 2) - (space * 2) - (1/5), 0)
             , ((boxWidth / 2) - (space * 2) - (1/5), 10) ]     
       |> (if model.click == ClickTab3 && model.tabState /= Tab3 then
            filled clickClr
          else
            filled hoverClr )
       |> if model.click == ClickTab3 then
            makeTransparent 1
          else if model.hoverTab == HoverTab3 && model.tabState /= Tab3 then 
            makeTransparent (tabFadeIn model.hoverTime)
          else makeTransparent 0
   , polygon [ ((boxWidth / 2) - (space * 2) - (1/5), 10)
             , ((boxWidth / 2) - (space * 2) - (1/5), 0)
             , ((boxWidth / 2) - space - (3/5), 0)
             , ((boxWidth / 2) - space - (3/5), 10) ]
       |> (if model.click == ClickTab4 && model.tabState /= Tab4 then
            filled clickClr
          else
            filled hoverClr )
       |> if model.click == ClickTab4 then
            makeTransparent 1
          else if model.hoverTab == HoverTab4 && model.tabState /= Tab4 then 
            makeTransparent (tabFadeIn model.hoverTime)
          else makeTransparent 0
   , polygon [ ((boxWidth / 2) - space - (3/5), 10)
             , ((boxWidth / 2) - space - (3/5), 0)
             , ((boxWidth / 2) - 1, 0)
             , ((boxWidth / 2) - 1, 10) ]          
       |> (if model.click == ClickTab5 && model.tabState /= Tab5 then
            filled clickClr
          else
            filled hoverClr )
       |> if model.click == ClickTab5 then
            makeTransparent 1
          else if model.hoverTab == HoverTab5 && model.tabState /= Tab5 then 
            makeTransparent (tabFadeIn model.hoverTime)
          else makeTransparent 0
   , tabLbls
                       ...
                       if ((mouseY >= 22.5 && mouseY <= 32.5) &&
                         ...
                         if ( mouseX >= (first (first tab1X)) &&
                           ...
                           { model | hoverTab = HoverTab2 
                                   , hoverExit = False
                                   , hoverTime = 0 }
                         else if ( mouseX >= (first (first tab3X)) &&
                                   mouseX <= (first (second tab3X)) &&
                                   model.tabState /= Tab3 ) then
                           { model | hoverTab = HoverTab3 
                                   , hoverExit = False
                                   , hoverTime = 0 }
                         else if ( mouseX >= (first (first tab4X)) &&
                                   mouseX <= (first (second tab4X)) &&
                                   model.tabState /= Tab4 ) then
                           { model | hoverTab = HoverTab4 
                                   , hoverExit = False
                                   , hoverTime = 0 }
                         else if ( mouseX >= (first (first tab5X)) &&
                                   mouseX <= (first (second tab5X)) &&
                                   model.tabState /= Tab5 ) then
                           { model | hoverTab = HoverTab5 
                                   , hoverExit = False
                                   , hoverTime = 0 }
                         else
                       ...
                       if (mouseY >= 22.5 && mouseY <= 32.5) then
                         if ( mouseX >= (first (first tab1X)) &&
                           ...
                           { model | click = ClickTab2
                                   ...
                                   mouseX <= (first (second tab3X)) &&
                                   model.tabState /= Tab3 ) then
                           { model | click = ClickTab3
                                   , hoverTab = HoverNone 
                                   , hoverExit = False }
                         else if ( mouseX >= (first (first tab4X)) &&
                                   mouseX <= (first (second tab4X)) &&
                                   model.tabState /= Tab4 ) then
                           { model | click = ClickTab4
                                   , hoverTab = HoverNone 
                                   , hoverExit = False }
                         else if ( mouseX >= (first (first tab5X)) &&
                                   mouseX <= (first (second tab5X)) &&
                                   model.tabState /= Tab5 ) then
                           { model | click = ClickTab5
                                   , hoverTab = HoverNone
                                   , hoverExit = False }
...
main = gameApp Tick { model = init, view = view, update = update, title = "MM | Info Menu" }
